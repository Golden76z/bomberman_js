...
export function canPickPowerUp(actor, powerUpElement) {
  ...
  let coordinates = getTilesCoordinates(
    actorInfo.positionX,
    actorInfo.positionY
  );
  ...
  const powerupTileX = Math.floor(
    parseInt(powerUpElement.style.left) / tileSize
  );
  const powerupTileY = Math.floor(
    parseInt(powerUpElement.style.top) / tileSize
  );
...
function removeWallsInRange(x, y, walls, map, owner) {
  ...
  if (map[y][x] === 0) {
    const centerWall = Wall.allWalls.find(
      (wall) => wall.tileX === x && wall.tileY === y && wall.type === 3
    );
    if (centerWall) {
  ...
  directions.forEach(([dy, dx]) => {
    for (let i = 1; i <= ownerBombLength; i++) {
      ...
      const wall = Wall.allWalls.find(
        (wall) => wall.tileX === newX && wall.tileY === newY
      );
  ...
  directions.forEach(({ dx, dy, dir }) => {
    for (let i = 1; i <= ownerBombLength; i++) {
      ...
      const isNextPositionOutOfBounds =
        nextY < 0 || nextY >= map.length || nextX < 0 || nextX >= map[0].length;

      // Then check if it's blocked (but only if it's in bounds)
      const isNextPositionBlocked =
        isNextPositionOutOfBounds || map[nextY][nextX] === 1;

      // Determine if this is the end of the explosion
      const isEnd =
        i === ownerBombLength || (i < ownerBombLength && isNextPositionBlocked);
      ...
      // Stop if hit a wall
      const wallAtPosition = Wall.allWalls.find(
        (w) => w.tileX === newX && w.tileY === newY
      );
      if (
        wallAtPosition &&
        (wallAtPosition.type === 1 || wallAtPosition.type === 2)
      ) {
        break;
      }
    }
...
function isInExplosionRange(
  ...
  for (const { dx, dy } of directions) {
    ...
    for (let i = 1; i <= explosionLength; i++) {
      ...
      const wall = Wall.allWalls.find(
        (w) => w.tileX === checkX && w.tileY === checkY
      );
